...
N_CLASSES = 5
TARGETS = ['cappella_s_sebastiano','papa_leone', 'pieta', 'porta_santa', 'unknown']
...
class ExhibitDataset(object):
    def __init__(self, path='dataset'):
        ...
        for path in image_paths:
            ...
            #print(path)
            img = Image.open(path)
            img_resized = img.resize([HEIGHT,WIDTH], PIL.Image.ANTIALIAS)
        ...
        self.trainX, self.testX, self.trainY, self.testY = train_test_split(self.images,
        	self.labels, test_size=0.2, random_state=45)
        ...
        print("unknown: {}".format(len([v for v in self.trainY if 'unknown' in TARGETS[np.argmax(v)]])))
        ...
        print("unknown: {}".format(len([v for v in self.testY if 'unknown' in TARGETS[np.argmax(v)]])))
...
def prettify_confusion_matrix(cm, ind=['True cappella', 'papa_leone', 'pieta', 'porta_santa', 'unknown'],
 cols=['Pred cappella', 'papa_leone', 'pieta', 'porta_santa', 'unknown']):
...
weights = {
    ...
    'w5': tf.Variable(tf.truncated_normal([3,3,128,128], stddev=0.1)),
    'wA': tf.Variable(tf.truncated_normal([3,3,128,256], stddev=0.1)),
    'wB': tf.Variable(tf.truncated_normal([3,3,256,256], stddev=0.1)),
    'wC': tf.Variable(tf.truncated_normal([3,3,256,512], stddev=0.1)),
    'wD': tf.Variable(tf.truncated_normal([3,3,512,512], stddev=0.1)),
    'wE': tf.Variable(tf.truncated_normal([3,3,512,512], stddev=0.1)),
    'wF': tf.Variable(tf.truncated_normal([3,3,512,512], stddev=0.1)),
    'w6': tf.Variable(tf.truncated_normal([2*2*512,1024], stddev=0.1)),# output of previous layer should be 7*7 image with 64 channels so flatten in final layer by multipling
...
biases = {
    ...
    'bA': tf.Variable(tf.constant(0.1, shape=[256])),
    'bB': tf.Variable(tf.constant(0.1, shape=[256])),
    'bC': tf.Variable(tf.constant(0.1, shape=[512])),
    'bD': tf.Variable(tf.constant(0.1, shape=[512])),
    'bE': tf.Variable(tf.constant(0.1, shape=[512])),
    'bF': tf.Variable(tf.constant(0.1, shape=[512])),
...
def apply_cnn(x, weights, biases, useBatchNorm=True, useDropout=True):
    ...
    # another set of   (CONV => RELU) * 2 => POOL
    h_convA = tf.nn.relu(conv2d(h_pool6_drop, weights['wA']) + biases['bA'])
    h_convA = tf.contrib.layers.batch_norm(h_convA, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convA
    print(h_convA)
    h_convB = tf.nn.relu(conv2d(h_convA, weights['wB']) + biases['bB'])
    h_convB = tf.contrib.layers.batch_norm(h_convB, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convB
    print(h_convB)
    h_poolB = tf.layers.max_pooling2d(h_convB, (2,2),2)
    print(h_poolB)

    # Add dropout
    h_poolB_drop = tf.nn.dropout(h_poolB, 0.9) if useDropout else h_poolB

    # another set of   (CONV => RELU) * 2 => POOL
    h_convC = tf.nn.relu(conv2d(h_poolB_drop, weights['wC']) + biases['bC'])
    h_convC = tf.contrib.layers.batch_norm(h_convC, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convC
    print(h_convC)
    h_convD = tf.nn.relu(conv2d(h_convC, weights['wD']) + biases['bD'])
    h_convD = tf.contrib.layers.batch_norm(h_convD, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convD
    print(h_convD)
    h_poolD = tf.layers.max_pooling2d(h_convD, (2,2),2)
    print(h_poolD)

    # Add dropout
    h_poolD_drop = tf.nn.dropout(h_poolD, 0.9) if useDropout else h_poolD

    # another set of   (CONV => RELU) * 2 => POOL
    h_convE = tf.nn.relu(conv2d(h_poolD_drop, weights['wE']) + biases['bE'])
    h_convE = tf.contrib.layers.batch_norm(h_convE, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convE
    print(h_convE)
    h_convF = tf.nn.relu(conv2d(h_convE, weights['wF']) + biases['bF'])
    h_convF = tf.contrib.layers.batch_norm(h_convF, is_training=is_train, center=True, scale=True) if useBatchNorm else h_convF
    print(h_convF)
    h_poolF = tf.layers.max_pooling2d(h_convF, (2,2),2)
    print(h_poolF)

    # Add dropout
    h_poolF_drop = tf.nn.dropout(h_poolF, 0.9) if useDropout else h_poolF

    # Fully connected layer
    print(h_poolF_drop)
    h_pool7_flat = tf.reshape(h_poolF_drop, [-1, 2*2*512]) # flatten
...
with tf.Session() as sess:
    ...
    if(isTrainingPhase):
        try:
            ...
            # Save checkpoint
            save_path = saver.save(sess, "serial/PopCNN4.ckpt")
            print("Model saved in file: %s" % save_path)
            # Save graph
            tf.train.write_graph(sess.graph, 'serial', 'PopCNN4.pbtxt')
        except KeyboardInterrupt:
            print "\nTraining Interrupted!"

            # Save checkpoint
            save_path = saver.save(sess, "serial/PopCNNX.ckpt")
            print("Model saved in file: %s" % save_path)
            # Save graph
            tf.train.write_graph(sess.graph, 'serial', 'PopCNNX.pbtxt')
    elif(isContinuingTraining):
        print("-------- RESUMING TRAINING FROM SERIALIZED POINT --------")

        # Restore model weights from previously saved model
        model_path = "serial/PopCNN3.ckpt"
        ...
        # Save checkpoint
        save_path = saver.save(sess, "serial/PopCNN3.ckpt")
        print("Model saved in file: %s" % save_path)
        # Save graph
        tf.train.write_graph(sess.graph, 'serial', 'PopCNN3.pbtxt')
    elif(isTesting):
        # Restore model weights from previously saved model
        model_path = "serial/PopCNN3.ckpt"
        saver.restore(sess, model_path)
